...
const renderRichText = (blocks: RichTextBlock[]): React.ReactNode => {
  return blocks.map((block: RichTextBlock, blockIndex: number) => {
    ...
    if (block.type === "paragraph") {
      return (
        <p key={blockIndex} className="text-gray-600 text-lg leading-9 mb-4 max-w-xl">
    ...
    if (block.type === "list") {
      return (
        <ul key={blockIndex} className="list-disc list-inside text-gray-600 text-lg leading-9 mb-4 max-w-xl">
...
export default function HomeSection() {
  ...
  // ✅ Detect hero media type (video or image)
  const heroIsVideo = (homeData as any)?.heroMime?.startsWith("video");
  const heroIsImage = (homeData as any)?.heroMime?.startsWith("image");

  const clients: ClientLogo[] = homeData?.clientLogos ?? [];
  const isoCertificates: IsoCertificate[] = homeData?.isoCertificates ?? [];
  ...
  return (
    <main className="w-full overflow-hidden">
      ...
      <section className="relative w-full h-[810px] bg-gray-900 overflow-hidden">

        {/* ✅ VIDEO hero */}
        {heroIsVideo && homeData?.heroImage && (
          <video
        ...
        {/* ✅ IMAGE hero */}
        {heroIsImage && homeData?.heroImage && (
          <div
        ...
        <div className="relative w-full overflow-hidden">
          ...
          {clients.length > 0 ? (
            <div className="flex items-center animate-marquee gap-20 w-max">
              {[...clients, ...clients].map((client: ClientLogo, index: number) => (
            ...
            <div className="flex items-center gap-20 py-4">
              {Array.from({ length: 5 }).map((_: unknown, i: number) => (
                <div key={i} className="h-16 w-28 bg-gray-200 animate-pulse rounded" />
              ))}
            </div>
          )}
      ...
      <section className="w-full bg-white">
        {homeData?.homeSections && homeData.homeSections.length > 0 ? (
          homeData.homeSections.map((service: HomeSectionType, index: number) => {
            const isEven = index % 2 === 0;

            return (
              <Link
                to={`/module/${service.title?.toLowerCase().replace(/\s+/g, "-")}`}
                key={service.id}
                className="grid lg:grid-cols-2 min-h-[500px] cursor-pointer hover:opacity-95 transition"
              >

                {/* Content Side */}
                <div className={`bg-[#f5f5f5] flex flex-col h-[500px] ${!isEven ? "lg:order-2" : ""}`}>
                  ...
                  <div className="overflow-y-auto px-8 lg:px-16 pb-16 flex-1">
                    {renderRichText(service.description as RichTextBlock[])}
                  </div>

                </div>

                {/* Media Side */}
                <div className={`h-[500px] ${!isEven ? "lg:order-1" : ""}`}>
                  {service.media ? (
                    service.mime.startsWith("video") ? (
                      <video
                        src={service.media}
                        autoPlay
                        muted
