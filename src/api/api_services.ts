...
export const getModulePage = async (slug: string) => {
  const json = await apiClient(ENDPOINTS.MODULE_PAGE(slug));
  ...
  return {
    ...
    whyChooseUsTitle: item.Why_Choose_Us_Title ?? "Why Choose Us",
    whyChooseUs: (item.Why_Choose_Us ?? []).map((card: any) => ({
      id: card.id,
      title: card.title ?? "",
      description: card.description ?? "",
      iconUrl: card.icon
        ? getMediaUrl(
            card.icon.formats?.thumbnail?.url ||
            card.icon.url
          )
        : "",
    })),

    videoTitle: item.Video_title ?? "See it in Action",
    videoUrl: item.Demo_Video ? getMediaUrl(item.Demo_Video.url) : "",
    videoMime: item.Demo_Video?.mime ?? "",

    featuredTitle: item.Featured_Title ?? "",
    featuredDescription: item.Featured_Description ?? "",
    featureItems: (item.featureitem ?? []).map((f: any) => ({
      id: f.id,
      points: f.points ?? "",
    })),
    featureImageUrl: item.FeatureImage
      ? getMediaUrl(
          item.FeatureImage.formats?.large?.url ||
          item.FeatureImage.formats?.medium?.url ||
